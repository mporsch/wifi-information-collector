...
#include <netlink/genl/genl.h> // for nla_parse
...
{
	// TODO create foreach loop
	int ret;
	struct cmd power_save = {
		.name = "power_save",
		.args = NULL,
		.help = "Retrieve power save state.",
		.cmd = NL80211_CMD_GET_POWER_SAVE,
		.nl_msg_flags = 0,
		.hidden = 0,
		.idby = CIB_NETDEV,
		.handler = get_power_save,
		.selector = NULL,
		.parent = NULL,
	};
	struct cmd station_dump = {
		.name = "station_dump",
		.args = NULL,
	...
	ret = handle_cmd(state, devidx, idby, command_idby, &power_save);
	if (ret)
		return ret;

	ret = handle_cmd(state, devidx, idby, command_idby, &station_dump);
	if (ret)
		return ret;
...
static void usage(void)
{
	int i;
	printf("\nusage:\n"
	       "  wifi-information-collector [-i<ifname>]\n\n");
}
